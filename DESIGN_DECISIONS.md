...
3. **API response**: If analytics are not available for a requested window, the API returns HTTP 404 with a descriptive message: _"analytics not yet computed; trigger a sync first"_ — which is accurate whether the data is missing or the window is genuinely unsupported.
